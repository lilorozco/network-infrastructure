...
* The Source address of the frame is learned, and the Port information is stored.

* The Destination address is checked against the list of known addresses.

* If the Destination is found in the list, the frame is sent to the associated Port.

* If the Destination address is not found, the frame is sent to all ports known to the switch, except the ingress port. That action is called “Flooding.” The frame is duplicated as many times as necessary to send a copy to every known port.
...
* “Who has 192.0.2.10?” (request)
* “I have 192.0.2.10” (reply)
* “X has 192.0.2.10” (reply as proxy)
* Probe: Give me all the addresses on this subnet
...
In a standard configuration, a single fabric could support 3000+ customers, each having 65,000 VxLANs.
